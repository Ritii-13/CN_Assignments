...
# client_socket.bind(('127.0.0.1', 5002))  # Client (sending) on port 5002
try:
...
def send_frame(frame_num):
    ...
    if should_drop_packet():
        print(f"\tEntity 2: Frame {frame_num} dropped.")
    ...
    client_socket.sendto(frame.to_string().encode(), ('127.0.0.1', 5002))
    print(f"\tEntity 2 sent frame: Seq={frame.seq}, Ack={frame.ack}, Timestamp={frame.timestamp}")
    stats.total_sent += 1
    timeout_manager.start_timer(frame_num, retransmit_frame)

# Function to retransmit a frame on timeout
def retransmit_frame(frame_num):
    print(f"\tEntity 2: Timeout occurred, retransmitting frames starting from {frame_num}")
    # Loop through all frames in the window starting from the timed-out frame
    frame = frame_num
    while frame != next_frame_to_send:
        send_frame(frame)  # This will restart the timer for each retransmitted frame
...
def client_thread():
    ...
    while stats.total_sent < TOT_PACKETS:
        ...
        try:
            ...
            print(f"Entity 2 received ACK for Seq={frame.ack}, Timestamp={frame.timestamp}")
            ...
            while between(ack_expected, frame.ack, next_frame_to_send):
                ...
                n_buffered -= 1
                stats.total_acks_recieved += 1

        except socket.timeout:
            print("")
            # retransmit_frame(next_frame_to_send)
